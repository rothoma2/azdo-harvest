...
@click.option('--download', '-d', is_flag=True, help='Download all found files')
@click.option('--output-dir', default='./downloads', help='Directory to save downloaded files')
@click.option('--verbose', '-v', is_flag=True, help='Show detailed results')
def search(search_term, organization, project, pat, file_only, repo_only, limit, download, output_dir, verbose):
...
def display_results(results, verbose=False):
    ...
    console.print()
    console.print(f"[bold green]✓[/bold green] Found [bold]{repo_count}[/bold] repositories and [bold]{file_count}[/bold] files")
    console.print()
...
def download_files_parallel(searcher, file_results, output_dir):
    ...
    """
    console.print(f"\n[bold cyan]Downloading {len(file_results)} files to {output_dir}...[/bold cyan]\n")
    ...
    def download_single_file(file_result):
        ...
        try:
            ...
            if '.' in filename:
                ...
                custom_filename = f"{safe_repo}__{name_part}__{hash_prefix}.{ext_part}"
            else:
                custom_filename = f"{safe_repo}__{filename}__{hash_prefix}"
    ...
    with Progress(
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            futures = {executor.submit(download_single_file, fr): fr for fr in file_results}
    ...
    # Display summary
    console.print()
    console.print(f"[bold green]✓[/bold green] Successfully downloaded: [bold]{len(downloaded)}[/bold] files")
    if failed:
        console.print(f"[bold red]✗[/bold red] Failed: [bold]{len(failed)}[/bold] files")
    
    # Show downloaded files
    if downloaded:
        console.print("\n[bold cyan]Downloaded files:[/bold cyan]")
        for file_result, path, file_hash in downloaded:
            console.print(f"  [green]✓[/green] {Path(path).name} [dim](hash: {file_hash})[/dim]")
    
    # Show errors if any
    if failed:
        console.print("\n[bold red]Failed downloads:[/bold red]")
        for file_result, error in failed[:5]:  # Show first 5 errors
            console.print(f"  [red]✗[/red] {file_result.filename}: {error}")
        if len(failed) > 5:
            console.print(f"  ... and {len(failed) - 5} more errors")
